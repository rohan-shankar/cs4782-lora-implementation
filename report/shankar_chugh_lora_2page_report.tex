...
We implemented \texttt{LoRALinear} for standard \texttt{nn.Linear} modules and \texttt{LoRAConv1D} for GPT-2's \texttt{Conv1D} attention projection. Both wrappers run the frozen base path and the low-rank residual path in parallel, then add the scaled residual.
