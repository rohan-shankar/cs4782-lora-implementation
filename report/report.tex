...
\section{Reflections}
This project showed that parameter-efficient fine-tuning depends on both the mathematical idea and the implementation details. The most important choices were which modules to replace, whether base weights were truly frozen, how $A$ and $B$ were initialized, and whether the task head remained trainable.

We also learned that reproduction under limited compute should focus on qualitative evidence, not only exact numerical matching. Our results reproduce LoRA's key trend: low-rank updates can achieve strong downstream performance with orders of magnitude fewer trainable parameters. Given more time, we would run multiple seeds, tune full fine-tuning baselines more carefully, test additional insertion targets such as key/output/MLP projections, and add LoRA weight merging to measure inference latency directly.
...
\onecolumn
\section*{Figures}
We decided to put the figures at the end to support the report without interrupting our narrative sections so it could be more clear that we were under the two page limit minus images.
